...
# transfer model from pytorch1.7 to pytorch1.2 for later onnx
...
class MobileNetV3(nn.Module):
    def __init__(self, n_class=9, input_size=224, dropout=0.8, mode='large', width_mult=1.0):    #####  dropout 0.8
...
def get_params():
    ...
    parser.add_argument('--resume' , default='/model_large_2.pth', type=str, metavar='PATH', help='path to latest checkpoint (default: none)')
    ...
    try:
        ...
        if args["resume"]:
            if os.path.isfile(args["resume"]):
                ...
                net.load_state_dict(checkpoint['state_dict_'])

                save_checkpoint(
                    {'state_dict_': net.state_dict(),  'best_prec1': max_val_acc, }, 
                     filename=os.path.join('/search/odin/xxx/zyresnet/A_onnx/' 'model_large_2_transfer.pth')
                    )
